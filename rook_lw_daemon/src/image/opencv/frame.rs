...
impl OpenCvFrame {
    ...
    pub fn new(mat: opencv::core::Mat) -> FrameResult<Self> {
        if mat.empty() {
            return Err(FrameError::Capture("Cannot create frame from empty Mat".to_string()));
        }
        Ok(Self { mat })
    }
...
impl Frame for OpenCvFrame {
    ...
    fn get_width(&self) -> FrameResult<usize> {
        ...
        if cols <= 0 {
            return Err(FrameError::ProcessingError("Frame has invalid width".to_string()));
        }
        Ok(cols as usize)
    }
    ...
    fn get_height(&self) -> FrameResult<usize> {
        ...
        if rows <= 0 {
            return Err(FrameError::ProcessingError("Frame has invalid height".to_string()));
        }
        Ok(rows as usize)
    }
}
